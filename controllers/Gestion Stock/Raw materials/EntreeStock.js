...
module.exports = class EntreeStock {
  ...
  async create({ body }, res) {
    try {
      ...
      products.map((product, i) => {
        checkEmpty(product);
        const doublon = products.filter(
          (x) => x?.productId == product?.productId
        ).length;
        if (doublon > 1)
          throw `Le produit de la ligne ${++i} est repris ${doublon} fois.`;
        productList.push({
          dateOperation: !dateOperation ? new Date() : new Date(dateOperation),
      ...
      successMessage(
        res,
        `${productList.length} ligne(s) affectée(s) avec succès.`
      );
    } catch (error) {
  ...
  async get({}, res) {
    try {
      const data = await prisma.entreeStock.findMany({
        where: { deleted: false },
        include: { MatierePremiere: { select: { designation: true } } },
  ...
  async getByID({ params }, res) {
    try {
      ...
      if (!data) throw `Aucune information...`;
  ...
  async update({ params, body }, res) {
    try {
      ...
      if (!entreeStock) throw `ID invalide...`;

      const kilo =
        entreeStock?.MatierePremiere?.kiloDisponible -
        entreeStock?.nombreKg +
        data?.nombreKg;
      await prisma.matierePremiere.update({
